...
async fn read_workflows(state: &AppState) -> anyhow::Result<Vec<Workflow>> {
...
async fn write_workflows(state: &AppState, workflows: &[Workflow]) -> anyhow::Result<()> {
    tokio::fs::write(workflows_path(state), serde_json::to_string_pretty(workflows)?).await?;
    Ok(())
}

pub async fn list(State(state): State<AppState>) -> impl IntoResponse {
    match read_workflows(&state).await {
...
pub async fn create(
    ...
    let mut wfs = match read_workflows(&state).await {
        Ok(w) => w,
    ...
    wfs.push(body.clone());
    match write_workflows(&state, &wfs).await {
